...
function getMAC(data) { //0 未扫描到设备, 1 匹配到设备, 2匹配到设备没有mac地址
	...
	if(data.deviceId === deviceId) {
		...
		if(data.manufacturerData) {
			let manufacturerData = data.manufacturerData;
			manufacturerData = advertisData.replace(/ /g, '');
			manufacturerData = manufacturerData.slice(manufacturerData.length - 13, manufacturerData.length - 1).toLocaleUpperCase();
			let macAdress = this.analysisMac(manufacturerData);
